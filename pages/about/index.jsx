import React from "react";


function About() {
  console.log("hello");
  return (
    <div className="">
      <div className="mybox1">
        <div className="myinfo">
          <div className="aboutme ">About.</div>
          <div class="mytext">
            <p>
              So I started to walk into the water. I won't lie to you boys, I
              was terrified. But I pressed on, and as I made my way past the
              breakers a strange calm came over me. I don't know if it was
              divine intervention or the kinship of all living things but I tell
              you Jerry at that moment, I <em>was</em> a marine biologist.
            </p>
          </div>
        </div>
        <div className="myimg">
          <img src="https://i.pinimg.com/474x/88/d5/99/88d599064d49fcc6219456cacf681b4f.jpg"></img>
        </div>
      </div>
      <div className="content">
        <div className="edu">
          <div className="vl"></div>
          <div className="experiece">
            <h3>Education</h3>
            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
              dsfbknjdsjlfbksdlvcdfbsdfhbad sdfbasdvads adsf adfvas asdvasd
              asdvas ZVASDGASD dfbksdvsjcnvlsknvlskz.dvnlkzsv.
            </li>

            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
            </li>
            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
              dsfbknjdsjlfbksdlvcdfbsdfhbad sdfbasdvads adsf adfvas asdvasd
              asdvas ZVASDGASD dfbksdvsjcnvlsknvlskz.dvnlkzsv.
            </li>
            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
            </li>
          </div>
        </div>
        <div className="edu">
          <div className="vl"></div>
          <div className="experiece">
            <h3>Experience</h3>
            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
              dsfbknjdsjlfbksdlvcdfbsdfhbad sdfbasdvads adsf adfvas asdvasd
              asdvas ZVASDGASD dfbksdvsjcnvlsknvlskz.dvnlkzsv.
            </li>

            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
            </li>
            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
              dsfbknjdsjlfbksdlvcdfbsdfhbad sdfbasdvads adsf adfvas asdvasd
              asdvas ZVASDGASD dfbksdvsjcnvlsknvlskz.dvnlkzsv.
            </li>
            <li>
              asdighlisudhg o;sb;s s oish;osidfvsd ;soidgh;soidfhg sdfghsdigh;s
              ;sodihosidhg ;voisdhfg;s a;osdiv;s ;a. sdfsdfbfbskdnv ksdfj
            </li>
          </div>
        </div>
      </div>
    </div>
  );
}

export default About;
